...
void PIDcal(void) {
  if (millis() - tcount > 3) {
    ...
    yawErrNow = yawReq - (ypr[0] * 180 / M_PI);   
    if (yawErrNow < -180) {
      yawErrNow += 360;
    }
    if (yawErrNow > 180) {
    ...
    yaw = (yawKp * yawErrNow) + (yawKd * (-(ypr[0] * 180 / M_PI) + yawErrPrev)) + (yawErrIgt * yawKi);
